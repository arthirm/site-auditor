...
module.exports = class Tracer {
  ...
  async trace() {
    ...
    try {
      ...
      // Wait for rumMarker event ot loadEventFired
      ...
      const timeout = new Promise((resolve, reject) => {
        timeoutId = setTimeout(() => {
          ...
          reject('Promise timed out after waiting for 60s');
        }, 600000);
      });

      // Timeout if either Rumamrker or loadEventFired does not happen in 60 secs
      await Promise.race([
        eventPromise,
        timeout
      ]).then(() => {
