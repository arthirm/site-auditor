...
class ConfigUtils {
  ...
  static validate(config) {
    const { debugPort, baseUrl, budgets, includeUrlPattern, printAuditSummary } = config;
    let msg = '';
    ...
    if (!printAuditSummary && (!config.assetManifest || !config.assetManifest.diffReport)) {
      msg += "Either printAuditSummary or diffReport should be configured to view the results"
    }
...
function getTable(columnsTitle) {
