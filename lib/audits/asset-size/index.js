 ...
 * @param {*} pageResults
 */
 module.exports = async function performAssetSizeCheck(page, pageResults) {
  ...
  if (failedAudits !== undefined && failedAudits.length > 0 && page.config.assetManifest) {
    page.config.assetManifest.includedTypes.forEach(async type => {
      ...
      if (
        ...
        failedAudits.filter(
          resource => resource.resourceType == type.toLowerCase()
        )
      ) {
