...
async function diffReport(results, failedResources) {
  return Promise.all(results.map(async r =>  {
    ...
    // To avoid generating diff report for pages that do not have source maps.
    const typesWithoutSourceMap = page.config.assetManifest.typesWithoutSourceMap;
    const filteredFailedResources = failedResources ? failedResources.filter(resource => includedTypes.includes(resource.resourceType) && !typesWithoutSourceMap.includes(resource.resourceType)) : [];
