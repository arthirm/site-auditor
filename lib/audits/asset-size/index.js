...
async function generateManifests(results) {
  return Promise.all(results.map(async r => {
    const page = r.page;
    await page.config.assetManifest.includedTypes.map(async type => {
...
async function diffReport(results, failedResources) {
  return Promise.all(results.map(async r => {
    const page = r.page;
    return page.config.assetManifest.includedTypes.map(async type => {
      // Print failure report if there are failed audits for resourcetype
      if ( failedResources !== undefined && failedResources.length > 0 &&
