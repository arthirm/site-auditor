...
async function diffReport(results, failedResources) {
  return Promise.all(results.map(async r => {
    ...
    if (failedResources !== undefined && failedResources.length > 0 && page.config.assetManifest) {
      await Promise.all(await computeDiff(page, r.pageResult, failedResources));
    } else {
...
async function computeDiff(page, pageResult, failedResources) {
  return page.config.assetManifest.includedTypes.map(async type => {
    ...
    if (page.config.assetManifest.diffReport &&
