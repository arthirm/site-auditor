 ...
 */
module.exports = class AuditBudget {
      ...
      return resourceSizes
        .map(resource => {
          ...
          if (summary[resourceType] !== undefined) {
            ...
            const sizeOverBudget =
              size > budgetinKB ? size - budgetinKB : undefined;
            return { url: this._page.relativePath, resourceType, size, sizeOverBudget };
          }
        })
  ...
  report() {
    ...
    let encoding;
    if (this._page.config.assetManifest && this._page.config.assetManifest.encoding) {
      encoding = this._page.config.assetManifest.encoding;
    }

    if (auditResults.length > 0) {
      let msg = `\nAudit summary for ${chalk.yellow(pathBudget[0].name)}`;
      msg = encoding ? `${msg} (${encoding} compressed)` : msg;

      console.log(chalk.blue.bold(`${msg} \n`));
      auditResults.forEach(resource => {
        if (resource.sizeOverBudget > 0) {
          let msg = `You exceeded the allowable maximum for a ${chalk.yellow(
            pathBudget[0].name
          )} ${chalk.yellow(resource.resourceType)}!
          Now the size is ${chalk.yellow(
            formatSize(resource.size)
          )} with a total increase of ${chalk.red(
            formatSize(resource.sizeOverBudget)
          )}`;
          log(logSymbols.error, msg);
        } else {
          let msg = `Asset size check passed for ${pathBudget[0].name} ${resource.resourceType}!`;
