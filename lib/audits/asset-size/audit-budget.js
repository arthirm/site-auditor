...
const { formatSize, getTable } = require('../../utils');
 ...
 * @param {*} auditResults
 */
function printSummaryAndGetFailedResources(auditResults, config) {
  ...
  const columns = ['Page', 'Type', 'Exceeded/Not', `Current size (${encoding})`, `Size Limits (${encoding})`, `Difference (${encoding})`, 'Url'];
      ...
      if (result.audits.length > 0) {
        result.audits.forEach(resource => {
          if (!printOnlyFailedPages || (printOnlyFailedPages && resource.sizeOverBudget > 0)) {
            table.push([pathBudget[0].name, resource.resourceType, resource.sizeOverBudget > 0 ? `Exceeded` : `No`,
...
module.exports = {
  AuditBudget, printSummaryAndGetFailedResources
}