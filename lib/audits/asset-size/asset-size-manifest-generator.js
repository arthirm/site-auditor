...
module.exports = class AssetSizeManifestGenerator {
  constructor(page, filetype, pageResult, auditSummary) {
    ...
    // Audit summary contains the total compressed size of the page
    this._auditSummary = auditSummary;
  ...
  /**
   * Check whether a fileType has source maps generated.
   */
  hasSourceMap() {
    const {
      includedTypes,
      typesWithoutSourceMap
    } = this._page.config.assetManifest;
    return includedTypes.includes(this._filetype) && !typesWithoutSourceMap.includes(this._filetype);
  }
  ...
  async createAssetManifests() {
    try {
      const hasSourceMap = this.hasSourceMap();
      let routeObj = {};
      routeObj.routeName = this._pathName;
      routeObj.filetype = this._filetype;
      routeObj.totalCompressedSize = formatSize(this._auditSummary.size);

      // If a fileType does not support sourcemaps then do not invoke source-map-explorer.
      // Create the json file only with the totalCompressedSize retrieved from tracing.
      if (hasSourceMap) {
        ...
        if (explorerResult) {
          ...
          routeObj.files.forEach(bundle => {
            ...
            totalUncompressedSize += bundle['totalBytes'] + bundle['unmappedBytes'];
          });
          routeObj.totalRawUncompressedSize = totalUncompressedSize;
          routeObj.totalUncompressedSize = formatSize(totalUncompressedSize);
