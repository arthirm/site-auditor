...
module.exports = class DiffReporter {
  ...
  printTable(results) {
    ...
    table.push(['Total', '', `${prev.toFixed(3)} KB`, `${curr.toFixed(3)} KB`, `${change.toFixed(3)} KB`]);
  ...
  computeTotal(results) {
    ...
    results.forEach(diff => {
      prev = diff.lhs ? (prev + parseFloat(formatSize(diff.lhs))) : prev;
      curr = diff.rhs ? (curr + parseFloat(formatSize(diff.rhs))) : curr;
      change += parseFloat(formatSize(diff.change));
    });
