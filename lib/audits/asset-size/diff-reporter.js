...
class DiffReporter {
  ...
  addBundleNameToDiff(manifest, diffResult) {
    ...
    if (
      this._config &&
      this._config.assetManifest &&
      this._config.assetManifest.buildDir
    ) {
    ...
    return diffResult.map(diff => {
      ...
      bundles.forEach(bundle => {
        if (bundle.files && Object.keys(bundle.files).includes(file)) {
          diff['bundleName'].push(
            bundle['bundleName'].replace(baseBundleDir, '')
          );
  ...
  diffReport() {
    ...
    if (fs.existsSync(this._lhs)) {
      ...
      const result = deepDiff.diff(
        this.flattenBundle(oldManifest),
      ...
      if (result === undefined) {
        ...
        const resultWithBundles = this.addBundleNameToDiff(
          currentManifest,
          result
        );
