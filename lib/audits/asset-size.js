  ...
  report() {
    ...
    try {
      ...
      if (auditResults === undefined) {
        console.error(`Error: Lighthouse budget result is undefined. Please check if the lighthouse was run successfully.
   ...
   */
   async createObjForRoute() {    
    try {
      ...
      error(`Error: Couldn't run source map explorer for the page '${this._path}'.
        Make sure the source maps exists for the file(s) '${this._filesArray}' \n ${JSON.stringify(e)}`);
    }
  }
