...
class DiffReporter {
  ...
  getTable() {
    const table = new Table({
      ...
      head: [ "Files", "Prev Size", "Current size", "Change"]
    });
    return table;
  }
  ...
  printFailureReportTable(groupedByEditKind, kind) {
    ...
    groupedByEditKind[kind].forEach(diff => {
      const file = Array.isArray(diff["path"])
          ...
          : diff["path"];
          if (kind === "E") {
            table.push([file, formatSize(diff.lhs), formatSize(diff.rhs), formatSize(diff.rhs - diff.lhs)]);
          } else if (kind === "N") {
            table.push([file, null, formatSize(diff.rhs), null]);
          } else if (kind === "D") {
            table.push([file, formatSize(diff.lhs)]);
          }
    });
        ...
        let pathName;
          try {
            ...
            if (page.config.assetManifest.diffReport && failedAudits.filter(resource => resource.resourceType == type.toLowerCase())) {
              ...
              const diffFinder = new DiffReporter(path.join(prevManifestTargetDir, assetFileName), path.join(currentManifestTargetDir, assetFileName), pathName);
