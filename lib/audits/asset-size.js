...
class BudgetReport {
  ...
  summarize(resourceSizes) {
    ...
    this._networkResponse = this._networkResponse.filter(obj => (obj.url ? obj.url.includes(includeUrlPattern) : false))
...
class AssetSizeManifestGenerator {
   ...
   */
   networkRequestToPaths() {
    const { includedTypes, buildDir, encoding } = this._page.config.assetManifest;
    const includeUrlPattern = this._page.config.includeUrlPattern;
